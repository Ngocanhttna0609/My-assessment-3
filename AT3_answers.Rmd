...
replicator_function<-function(NMUT){
  mean(replicate(n=150,rep_mut(NMUT)))
}
...
Now we can construct the increasing proportion of mutated bases by the decreasing the ability for BLAST to match the gene sequence of origin. In here, we can conclude that from the orginal sequence that is 615 bp 20% of randomised bases in the total sequence have 69% success matches with BLAST but with 40% of the randomised bases gave us 0% successful BLAST. This highlights the limitations of BLASTs to give successful matches when sites in the sequence are randomised we can say that about 30-40% randomised bases BLAST becomes unsuccesful. 
```{r, chart}
#To create a chart we can make a scatterplot of the proportions of sites and succesful BLASTs.

#The proportion of sites randomised from the total sequence length of 615 bp, 150 repeats using gene 1.

proportion_plot<-n_mut_range/length_seq

#The proportion of successful BLASTs from the previous result
prop_blast<-c(myresult)

plot(proportion_plot,prop_blast, 
     type='l', 
